...
cats = ["Any"] + sorted(list({q.get("category","General") for q in qs_all})) if qs_all else ["Any"]
...
if state and state["questions"]:
    ...
    if st.button("Submit"):
        ...
        for i, q in enumerate(state["questions"]):
            ...
            st.markdown(f"- **Q{i+1} Answer:** {ans}")
        st.session_state.quiz_state = None
elif state is not None and not state["questions"]:
